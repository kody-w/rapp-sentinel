...
| continuity that migrates | the creative ledger's current cycle is read canonically — `cycle`, else `last_cycle`, else a validated `cycles[]` — and fields that disagree fail closed rather than guessing. Legacy history is carried forward, never flattened, and a rejected attempt is a failed spend that leaves public continuity alone |
